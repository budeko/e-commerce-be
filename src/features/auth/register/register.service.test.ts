...
vi.mock('../../../lib/auth/unverified-user', () => ({
...
const { register } = await import('./register.service');
...
describe('register', () => {
  ...
  it('cooldown aktifken 429 döner', async () => {
    const { RegisterError } = await import('./register.errors');

    mockAssertRegisterEmailCooldown.mockRejectedValue(
      new RegisterError(429, 'E-posta gönderimleri arasında bekleme süresi var')
    );
