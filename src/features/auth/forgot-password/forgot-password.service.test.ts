...
vi.mock('../../../lib/auth/email-token', () => ({
  signPasswordResetToken: vi.fn().mockReturnValue('reset-token'),
}));

vi.mock('../../../lib/auth/email/send-mail', () => ({
  sendPasswordResetEmail: (...args: unknown[]) => mockSendPasswordResetEmail(...args),
}));

vi.mock('../../../lib/auth/email-cooldown', async () => {
  const actual = await vi.importActual<typeof import('../../../lib/auth/email-cooldown')>(
    '../../../lib/auth/email-cooldown'
  );
...
const { EmailCooldownError } = await import('../../../lib/auth/email-cooldown');
const { forgotPassword } = await import('./forgot-password.service');
