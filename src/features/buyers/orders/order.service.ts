...
const assertSellerStatusTransition = (currentStatus: OrderStatus, nextStatus: OrderStatus) => {
  if (nextStatus === 'shipped' && currentStatus !== 'paid') {
    throw new CommerceError(400, 'Sipariş yalnızca paid durumundayken shipped yapılabilir');
  }

  if (nextStatus === 'delivered' && currentStatus !== 'shipped') {
    throw new CommerceError(400, 'Sipariş yalnızca shipped durumundayken delivered yapılabilir');
  }
};
